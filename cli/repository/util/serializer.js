...
class Serializer {
    static classifyQuads(quadC, contactUrlQuads, nickNameQuads) {
    ...
    static rebuildContacts(contactUrlQuads, nickNameQuads) {
        const contacts = [];
        contactUrlQuads.forEach(contactUrlQuad => {
            ...
            nickNameQuads.forEach(nickNameQuad => {
                ...
                if (subjectValue === objectValue) {
                    const contact = new contact_1.Contact(contactUrlQuad.object.value, nickNameQuad.object.value);
            ...
            if (nickControl === false) {
                const contact = new contact_1.Contact(contactUrlQuad.object.value, contactUrlQuad.object.value.split('/')[2]);
                const arrayContact = contact.urlPod.split('/');
                const urlFormatted = arrayContact[0] + '//' + arrayContact[2] + '/';
                contact.urlPod = urlFormatted;
                contacts.push(contact);
...
Serializer.serializeUnknownContacts = (contacts) => {
...
Serializer.deserializeFolderNameFiles = (data) => __awaiter(this, void 0, void 0, function* () {
    const names = [];
    if (data != null && data.trim() !== '') {
        const parser = new N3.Parser();
        let i = 0;
        parser.parse(data, (error, quadC, prefixes) => {
            if (error) {
                i = 1;
            }
            if (quadC) {
        ...
        while (i === 0) {
            const e = yield new Promise(resolve => setTimeout(resolve, 1000));
        }
    }
    return names;
});
Serializer.deserializeNotification = (data) => {
    let notificaction = null;
    if (data != null && data.trim() !== '') {
        const objJSON = JSON.parse(data);
        const chatIdentificator = objJSON['_chatIdentificator'];
        const from = new contact_1.Contact(objJSON['_message']['_from']['_urlPod'], objJSON['_message']['_from']['_nickname']);
        const to = new contact_1.Contact(objJSON['_message']['_to']['_urlPod'], objJSON['_message']['_to']['_nickname']);
        const text = objJSON['_message']['_text'];
        const date = new Date(objJSON['_message']['_date']);
        const messg = new message_1.Message(from, to, date, text);
        ...
        notificaction = new notification_1.Notification(chatIdentificator, messg);
        return notificaction;
    }
    return notificaction;
};
Serializer.deserializeMessages = (data) => {
    const messages = [];
    ...
    for (let i = 0; i < objJSON.length; ++i) {
        const from = new contact_1.Contact(objJSON[i]['_from']['_urlPod'], null);
        const to = new contact_1.Contact(objJSON[i]['_to']['_urlPod'], null);
        const messg = new message_1.Message(from, to, new Date(objJSON[i]['_date']), objJSON[i]['_text']);
...
Serializer.deserializeUnknownContacts = (data) => {
    const contacts = [];
    if (data == null || data.length === 0) {
        return contacts;
    }
    const objJSON = JSON.parse(data);
    for (let i = 0; i < objJSON.length; ++i) {
        const contact = new contact_1.Contact(objJSON[i]['_urlPod'], 'Unknown');
        contact.isUnknown = true;
        contacts.push(contact);
    }
    return contacts;
};
Serializer.serializeContact = (newContact, oldData) => __awaiter(this, void 0, void 0, function* () {
    const writer = new N3.Writer();
    let i = 0;
    const parsePromisePrefixes = new Promise((resolve, reject) => {
        const parser = new N3.Parser();
        parser.parse(oldData, (error, quadC, prefixes) => {
            if (error) {
                reject(error);
            }
            if (prefixes) {
        ...
        parser.parse(oldData, (error, quadC, prefixes) => {
            if (quadC) {
                ...
                if (newContact.nickname != null && newContact.nickname.trim() !== '') {
                    writer.addQuad(namedNode(newContact.urlPod.split('/')[2].replace(/\./gi, '') + ':me'), namedNode('n0:nick'), literal(newContact.nickname));
                }
                resolve('Finish');
            }
        });
...
Serializer.serializeUpdateContact = (updateContact, oldData) => __awaiter(this, void 0, void 0, function* () {
    return yield Serializer.serializeManageContact(updateContact, oldData, true);
});
Serializer.serializeManageContact = (changeContact, oldData, modify) => __awaiter(this, void 0, void 0, function* () {
                ...
                if (quadC) {
                    ...
                    if (quadC.object.value !== urlContact && quadC.object.value !== urlContact + 'me'
                        ...
                        if (modify) {
                            if (quadC.predicate.value === 'http://xmlns.com/foaf/0.1/nick') {
                                controlHad = true;
                                writer.addQuad(namedNode(quadC.subject.value), namedNode('n0:nick'), literal(changeContact.nickname));
                    ...
                    if (modify) {
                        if (!controlHad) {
                            writer.addQuad(namedNode(subjectValue + '/me'), namedNode('n0:nick'), literal(changeContact.nickname));
                        }
...
Serializer.serializeDeleteContact = (delContact, oldData) => __awaiter(this, void 0, void 0, function* () {
    return yield Serializer.serializeManageContact(delContact, oldData, false);
});
Serializer.deserializeContacts = (data) => __awaiter(this, void 0, void 0, function* () {
    const parser = new N3.Parser();
...
Serializer.deserializeImageContacts = (data) => __awaiter(this, void 0, void 0, function* () {
    if (data != null) {
        const parser = new N3.Parser();
        let i = 0;
        let image = null;
